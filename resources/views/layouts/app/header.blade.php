...
<body class="min-h-screen bg-zinc-50 dark:bg-zinc-900">
    ...
    <header
        class="sticky top-0 z-40 w-full border-b border-[#E5E8EF] bg-white dark:bg-[#1B1A1C] dark:border-zinc-800 px-6 py-4 shadow-sm transition-all duration-300">
        <div class="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <nav class="hidden items-center space-x-1 text-center md:flex">
                @auth
                @if (auth()->user()->role === 'admin' || auth()->user()->role === 'superadmin')
                {{-- 🔒 ADMIN LINKS --}}
                ...
                {{-- Verify Now — resident only, label + dot color reflect current status --}}
                @php
                $navVerification = \App\Models\ResidenceVerification::where('user_id', auth()->id())->first();
                $navVerifyLabel = match($navVerification?->status) {
                'verified' => 'Verification',
                'pending' => 'Verification',
                'rejected' => 'Verification',
                default => 'Verify Now',
                };
                $navVerifyDotColor = match($navVerification?->status) {
                ...
                <a href="{{ route('verification') }}" wire:navigate
                    class="inline-flex items-center gap-1.5 text-sm px-4 py-2 rounded-lg transition-all duration-200
        {{ request()->routeIs('verification') ? 'text-[#1D74E3] bg-[#1D74E3]/10 font-bold' : 'text-[#33333B] hover:bg-[#E5E8EF]/50 hover:text-[#1D74E3] dark:text-zinc-300 dark:hover:bg-white/10 dark:hover:text-[#1D74E3] font-medium' }}">
                    @if ($navVerifyDotColor)
                    <span class="w-2 h-2 rounded-full shrink-0 {{ $navVerifyDotColor }}"></span>
                    @endif
                    {{ $navVerifyLabel }}
                </a>
