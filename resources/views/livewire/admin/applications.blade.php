<div class="min-h-screen bg-[#E5E8EF] dark:bg-[#1B1A1C]">
    <div class="max-w-7xl mx-auto p-8">
        ...
        {{-- Alert Messages --}}
        {{-- Using public Livewire properties instead of session()->flash() --}}
        {{-- session()->flash() is unreliable in Livewire because it uses AJAX, not full page reloads --}}
        @if ($successMessage)
        <div class="mb-6 bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded shadow-sm flex items-center space-x-2">
            <span class="font-semibold">Success:</span>
            <span>{{ $successMessage }}</span>
        </div>
        @endif
        @if ($infoMessage)
        <div class="mb-6 bg-blue-50 border-l-4 border-[#1D74E3] text-blue-700 p-4 rounded shadow-sm flex items-center space-x-2">
                    ...
                    <tr class="border-b border-gray-100 dark:border-zinc-700 hover:bg-[#E5E8EF]/50 dark:hover:bg-zinc-700/50 transition duration-150">
                        ...
                        <td class="p-4 text-right whitespace-nowrap space-x-2">
                            <button wire:click="viewDetails({{ $application->id }})"
                                class="bg-[#1D74E3] hover:bg-[#155ab2] text-white px-3 py-1.5 rounded-lg font-medium text-xs transition inline-flex items-center space-x-1 shadow-sm">
                                <span>👁️ View Details</span>
                            </button>
                            {{-- Approve now opens a confirmation modal first --}}
                            <button wire:click="openApprovalModal({{ $application->id }})"
                                class="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg font-medium text-xs transition inline-flex items-center space-x-1 shadow-sm">
                                <span>✓ Approve</span>
                            </button>
                            <button wire:click="openRejectionModal({{ $application->id }})"
                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg font-medium text-xs transition inline-flex items-center space-x-1 shadow-sm">
                                <span>✗ Reject</span>
                            </button>
                        </td>
                    </tr>
        ...
        {{-- ============================================================ --}}
        {{-- DETAILS MODAL --}}
        {{-- $selectedApplication is a view variable passed from render(), --}}
        {{-- NOT a public Livewire property — this avoids losing eager-loaded --}}
        {{-- relationships (scholarship.requirements, answers) on re-hydration. --}}
        {{-- ============================================================ --}}
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl max-w-2xl w-full flex flex-col max-h-[85vh]">
                ...
                <div class="p-6 border-b border-gray-100 dark:border-zinc-700 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-[#AA9A98] dark:text-zinc-400 mt-1">
                            Submitted by <strong class="text-gray-700 dark:text-zinc-200">{{ $selectedApplication->user->name }}</strong>
                            for <strong class="text-[#1D74E3]">{{ $selectedApplication->scholarship->title }}</strong>
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-700 border-t-4 border-t-transparent hover:border-t-[#1D74E3] hover:shadow-md transition-all duration-300">
                            ...
                            <div class="mt-2 text-sm text-[#1B1A1C] dark:text-white">
                                ...
                                <a href="/storage/{{ $answer->file_path }}" target="_blank"
                                    class="inline-flex items-center space-x-2 text-[#1D74E3] hover:underline font-medium bg-[#1D74E3]/10 px-3 py-1.5 rounded-lg border border-[#1D74E3]/20">
                ...
                <div class="p-6 border-t border-gray-100 dark:border-zinc-700 flex justify-between bg-white dark:bg-zinc-800 rounded-b-lg">
                    <button wire:click="closeDetailsModal"
                        class="px-4 py-2 border border-gray-200 dark:border-zinc-700 text-sm font-semibold text-gray-500 dark:text-zinc-400 hover:text-[#33333B] dark:hover:text-white rounded-lg transition">
                        Close
                    </button>
                    <div class="flex space-x-2">
                        <button wire:click="openRejectionModal({{ $selectedApplication->id }})"
                            class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition">
                            Reject Application
                        </button>
                        {{-- Approve also goes through confirmation modal --}}
                        <button wire:click="openApprovalModal({{ $selectedApplication->id }})"
                            class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition">
                            Approve Application
                        </button>
                    </div>
                </div>
        ...
        {{-- Shows who is being approved before committing. --}}
        {{-- ============================================================ --}}
        @if ($showApprovalModal && $pendingApprovalApplication)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6">
                    ...
                    <h3 class="text-lg font-bold text-[#33333B] dark:text-white text-center mb-1">
                        Confirm Approval
                    </h3>
                    <p class="text-sm text-[#AA9A98] dark:text-zinc-400 text-center mb-1">
                        You are about to approve the application of:
                    </p>
                    <p class="text-sm font-semibold text-[#1B1A1C] dark:text-white text-center mb-1">
                        {{ $pendingApprovalApplication->user->name }}
                    </p>
                    <p class="text-xs text-[#AA9A98] dark:text-zinc-400 text-center mb-5">
                        for <span class="text-[#1D74E3] font-medium">{{ $pendingApprovalApplication->scholarship->title }}</span>
                    </p>

                    {{-- Warning --}}
                    <div class="text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-3 text-center mb-6">
                        ⚠️ This will mark the application as <strong>approved</strong> and consume one scholarship slot. This cannot be undone.
                    </div>

                    <div class="flex justify-end gap-3">
                        <button wire:click="closeApprovalModal"
                            class="px-4 py-2 text-sm font-semibold text-gray-500 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-white border border-gray-200 dark:border-zinc-700 rounded-lg transition">
                            Cancel
                        </button>
                        <button wire:click="confirmApprove"
                            class="px-4 py-2 text-sm font-semibold bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-sm transition">
                            Yes, Approve
                        </button>
                    </div>
        ...
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6">
                    ...
                    <p class="text-xs text-[#AA9A98] dark:text-zinc-400 mb-4">
                        Please provide detailed remarks outlining the reason for rejection.
                        This will be logged and displayed to the student.
                    </p>

                    <form wire:submit.prevent="reject">
                        <div class="mb-4">
                            <label class="block text-xs font-semibold text-[#33333B] dark:text-zinc-350 mb-2 uppercase tracking-wider">
                                Rejection Remarks
                            </label>
                            <textarea wire:model="rejectionRemarks" rows="4"
                                class="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white rounded-lg p-3 text-sm focus:ring-[#1D74E3] focus:border-[#1D74E3] focus:outline-none"
                                placeholder="e.g., Your submitted grades do not meet the minimum GPA requirement of 85."></textarea>
